...
> **7점 이상 → LONG / -3점 이하 → SHORT / 그 사이 → NEUTRAL**
...
| 조건 | LONG | SHORT |
|------|------|-------|
| 1시간봉 신호 | LONG (RSI 35 이하 + BB 50% 이하 포함) | SHORT (RSI 80 이상 + BB 50% 이상 포함) |
...
- **매매 전략 전면 개편**: 모멘텀 추종 → 극단 구간 역추세 방식으로 변경
  - RSI 가중치 ±2 → ±3, LONG 기준 35 이하 / SHORT 기준 75 이상 (더 보수적 진입)
  - EMA 배열 가중치 ±3 → ±2
...
- **레버리지 정규화 수정**: 최대 점수 ±10 기준으로 `/10.0` 보정
